...
- Used staging Worker version `565fe6cf-5280-454e-8518-a7683672e253` briefly as
  a device-only bootstrap relay, then removed it after the owner-approved
  Wi-Fi-only install acknowledged `0.1.4` as applied; the tracker now connects
  directly to the home server and no BikeBoss runtime path uses Worker compute
